...
- 2021.07.24(上午):这周头痛没太多进展，这次主要是重新自己使用了https://github.com/riscv/riscv-gnu-toolchain（gitee镜像：https://gitee.com/mirrors/riscv-gnu-toolchain），仓库分别编译了用于linux的编译器和用于裸机开发的编译器，这样裸机开发就能使用newlib的c库。不过更新了编译器后重新编译target_root_app时更新了一些编译脚本，大部分都没有问题了，但是screen工作不正常，debug了一下还没找到问题，后面有空再查找这个问题了。
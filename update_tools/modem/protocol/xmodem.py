...
class XMODEM(Modem):
    ...
    def _send_packet(self, sequence, data, packet_size, crc_mode, crc,
        ...
        while True:
            ...
            self.putc(sequence.to_bytes(1, 'little'))
            self.putc((0xff - sequence).to_bytes(1, 'little'))
            self.putc(data)
            if crc_mode:
                self.putc((crc >> 8).to_bytes(1, 'little'))
                self.putc((crc & 0xff).to_bytes(1, 'little'))
            else:
                # Send CRC or checksum
                self.putc(crc.to_bytes(1, 'little'))

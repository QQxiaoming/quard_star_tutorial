...
class ZMODEM(Modem):
    ...
    def _recv_16_data(self, timeout):
        ...
        while char < 0x100:
            ...
            if char is TIMEOUT:
                return TIMEOUT, ''
            if char < 0x100:
                mine = self.calc_crc16((char & 0xff).to_bytes(1, 'little'), mine)
                data.append(char.to_bytes(1, 'little'))

        # Calculate our crc, unescape the sub_frame_kind
        sub_frame_kind = char ^ ZDLEESC
        mine = self.calc_crc16(sub_frame_kind.to_bytes(1, 'little'), mine)
    ...
    def _recv_32_data(self, timeout):
        ...
        while char < 0x100:
            ...
            if char is TIMEOUT:
                return TIMEOUT, ''
            if char < 0x100:
                mine = self.calc_crc32((char & 0xff).to_bytes(1, 'little'), mine)
                data.append(char.to_bytes(1, 'little'))

        # Calculate our crc, unescape the sub_frame_kind
        sub_frame_kind = char ^ ZDLEESC
        mine = self.calc_crc32(sub_frame_kind.to_bytes(1, 'little'), mine)
    ...
    def _recv_bin16_header(self, timeout):
        ...
        for x in xrange(0, 5):
            ...
            if char is TIMEOUT:
                return 0, False
            mine = self.calc_crc16(char.to_bytes(1, 'little'), mine)
            header.append(char)
    ...
    def _recv_bin32_header(self, timeout):
        ...
        for x in xrange(0, 5):
            ...
            if char is TIMEOUT:
                return 0, False
            mine = self.calc_crc32(char.to_bytes(1, 'little'), mine)
            header.append(char)
    ...
    def _send(self, char, timeout, esc=True):
        if char == ZDLE:
            ...
            self.putc(char.to_bytes(1, 'little'), timeout)
    ...
    def _send_esc(self, char, timeout):
        self.putc(ZDLE.to_bytes(1, 'little'), timeout)
        self.putc((char ^ 0x40).to_bytes(1, 'little'), timeout)
    ...
    def _send_hex_header(self, header, timeout):
        self.putc(ZPAD.to_bytes(1, 'little'), timeout)
        self.putc(ZPAD.to_bytes(1, 'little'), timeout)
        self.putc(ZDLE.to_bytes(1, 'little'), timeout)
        self.putc(ZHEX.to_bytes(1, 'little'), timeout)
        mine = 0
